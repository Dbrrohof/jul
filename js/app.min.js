var config={apiKey:"AIzaSyBAnks7FMf2TNQE8m7z_oh9Vs-mbs16Okw",authDomain:"julelodtraekning.firebaseapp.com",databaseURL:"https://julelodtraekning.firebaseio.com",projectId:"julelodtraekning",storageBucket:"julelodtraekning.appspot.com",messagingSenderId:"490164122312"};firebase.initializeApp(config);var usersArray=[];var wishlistsArray=[];var bucketArray=[];var database=firebase.database();var storage=firebase.storage();var usersLeft=[];var users=firebase.database().ref('users');var bucket=firebase.database().ref('hat').orderByValue().equalTo(0);var wishlists=firebase.database().ref('wishlists');users.on('value',function(snapshot){Object.entries(snapshot.val()).forEach(([key,value])=>{if(!value.nisse)usersLeft[key]=value});});users.on('value',function(snapshot){let obj=snapshot.val();});wishlists.on('value',function(snapshot){wishlistsArray=snapshot.val();getWishlists();});var currentUser=sessionStorage.getItem("name");if(currentUser){document.getElementById('hello').innerHTML="Hej <span class='name'>"+currentUser+"</span>!";getNisse();}else{window.location="signup"}
function getNisse(){firebase.database().ref('/users/'+currentUser+'/nisse').once('value').then(function(snapshot){if(snapshot.val()=="none"||snapshot.val()==0||snapshot.val()==null){document.getElementById('answer').innerHTML="???"}else{document.getElementById('answer').innerHTML="Du er nisse for <span class='name'>"+snapshot.val()+"</span>";document.getElementById('nisseknap').style.display='none';document.getElementById('nisseknap').value="Du er en nisse!";}});}
function logOut(){sessionStorage.clear();currentUser=null;window.location="signup"}
function getAnswer(){document.getElementById('nisseknap').style.display='none';bucket.once('value',function(snapshot){bucketArray=Object.keys(snapshot.val());getAnswerReady();});}
function getAnswerReady(){console.log(bucketArray);let keys=Object.keys(bucketArray);let chosenNisse=bucketArray[keys[keys.length*Math.random()<<0]];console.log(chosenNisse)
...
if(keys.length==2){for(let i=0;i<bucketArray.length;i++){if(usersLeft[bucketArray[i]]){if(bucketArray[i]!=currentUser){chosenNisse=bucketArray[i];}}}}
...
function getWishlists(){let listOfWishlists=document.getElementById('wishlistsInner');listOfWishlists.innerHTML="";var object=[];for(var key in wishlistsArray){let listItem=document.createElement('li')
...
function getNamesLeft(){firebase.database().ref('hat').orderByValue().equalTo(0).once('value').then(function(snapshot){});}
