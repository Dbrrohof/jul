var config={apiKey:"AIzaSyBAnks7FMf2TNQE8m7z_oh9Vs-mbs16Okw",authDomain:"julelodtraekning.firebaseapp.com",databaseURL:"https://julelodtraekning.firebaseio.com",projectId:"julelodtraekning",storageBucket:"julelodtraekning.appspot.com",messagingSenderId:"490164122312"};firebase.initializeApp(config);var usersArray=[];var wishlistsArray=[];var bucketArray=[];var database=firebase.database();var storage=firebase.storage();var users=firebase.database().ref('users');var bucket=firebase.database().ref('hat').orderByValue().equalTo(0);var wishlists=firebase.database().ref('wishlists');users.on('value',function(snapshot){usersArray=snapshot.val();});wishlists.on('value',function(snapshot){wishlistsArray=snapshot.val();getWishlists();});var currentUser=sessionStorage.getItem("name");if(currentUser){document.getElementById('hello').innerHTML="Hej "+currentUser+"!";getNisse();}else{window.location="signup.html"}
function getNisse(){firebase.database().ref('/users/'+currentUser+'/nisse').once('value').then(function(snapshot){if(snapshot.val()=="none"||snapshot.val()==0||snapshot.val()==null){document.getElementById('answer').innerHTML="???"}else{document.getElementById('answer').innerHTML="Du er nisse for "+snapshot.val();document.getElementById('nisseknap').disabled=true;document.getElementById('nisseknap').value="Du er en nisse!";}});}
...
firebase.database().ref('users/'+currentUser).update({nisse:chosenNisse});document.getElementById('answer').innerHTML="Du er nisse for "+chosenNisse;firebase.database().ref('hat/').update({[chosenNisse]:1});}
